...
function MandalaGeometry({ breakProgress }: MandalaGeometryProps) {
  ...
  useFrame(() => {
    ...
    // Use breakProgress directly from parent (already mapped to 0-1 for breaking phase)
    // breakProgress is 0 when shape is intact, 1 when fully broken
    ...
    groupRef.current.children.forEach((child, index) => {
      ...
      material.opacity = 1 - breakProgress; // Fades from 1 to 0
...
export function GeometricWireframe({ breakProgress }: GeometricWireframeProps) {
  return (
    ...
    >
      ...
      <MandalaGeometry breakProgress={breakProgress} />
    </Canvas>
  );
}
